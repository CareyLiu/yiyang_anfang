...
public class TuyaBaseCameraDeviceActivity extends BaseActivity {

    public void setDp(String key, Object value) {
        Map<String, Object> dps = new HashMap<>();
        dps.put(key, value);
        TuyaDeviceManagerTwo.getDeviceManager().getDevice().publishDps(TuyaDeviceManagerTwo.getDeviceManager().getJosnString(dps), new IResultCallback() {
            @Override
    ...
    public void getDp(String dpId) {
        TuyaDeviceManagerTwo.getDeviceManager().getDevice().getDp(dpId, new IResultCallback() {
            @Override
            public void onError(String code, String error) {
